...
class App extends React.Component {
  ...
  getRemainingTime = () => {
    ...
    const setCompletionTime = ((targetIntervals * intervalTime) + ((targetIntervals - 1) * restTime))
  ...
  render() {
    ...
    const { currentInterval, currentTime, done, resting, targetIntervals } = this.state
    return (
      <div className={classes.global_styles}>
        <Layout>
          <Grid item xs={12}>
            <h2>Interval {currentInterval} of {targetIntervals}</h2>
            <Clock
              done={done}
              resting={resting}
